...
import org.monkey.mmq.core.cluster.lookup.LookupFactory;
import org.monkey.mmq.core.common.Commons;
...
/**
 * Cluster node management in mmq.
 *
...
public class ServerMemberManager implements ApplicationListener<WebServerInitializedEvent> {
    ...
    private static final String MEMBER_CHANGE_EVENT_QUEUE_SIZE_PROPERTY = "mmq.member-change-event.queue.size";
    ...
    protected void init() throws MmqException {
        Loggers.CORE.info("Mmq-related cluster resource initialization");
        ...
        if (serverList.isEmpty()) {
            throw new MmqException(MmqException.SERVER_ERROR, "cannot get serverlist, so exit.");
        }
        
        Loggers.CORE.info("The cluster resource is initialized");
    }
    
    private void initAndStartLookup() throws MmqException {
     ...
     * @throws MmqException exception.
     */
    public void switchLookup(String name) throws MmqException {
     ...
     */
    @PreDestroy
    public void shutdown() throws MmqException {
        serverList.clear();
