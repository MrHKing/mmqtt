...
public class VersionUtils {
    ...
    private static final String MMQ_VERSION_FILE = "mmq-version.txt";
    
    static {
        InputStream in = null;
        try {
            in = VersionUtils.class.getClassLoader().getResourceAsStream(MMQ_VERSION_FILE);
            Properties props = new Properties();
            props.load(in);
            String val = props.getProperty("version");
            if (val != null && !VERSION_PLACEHOLDER.equals(val)) {
                version = val;
                clientVersion = "Mmq-Java-Client:v" + VersionUtils.version;
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
