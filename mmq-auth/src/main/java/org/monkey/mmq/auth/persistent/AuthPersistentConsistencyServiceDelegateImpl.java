...
import static org.monkey.mmq.core.common.Constants.MQTT_PERSISTENT_AUTH_GROUP;
 ...
 * @author solley
 */
@Component("authPersistentConsistencyServiceDelegateImpl")
public class AuthPersistentConsistencyServiceDelegateImpl implements PersistentConsistencyService {

    private final BasePersistentServiceProcessor persistentConsistencyService;

    private final String baseDir;

    public AuthPersistentConsistencyServiceDelegateImpl(ProtocolManager protocolManager)
            throws Exception {
    ...
    protected Class<? extends Record> getClassOfRecordFromKey(String key) {
        if (KeyBuilder.matchAuthKey(key)) {
            return User.class;
        }
        return Record.class;
    }
